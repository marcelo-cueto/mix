<body>
    <div class="wrapper d-flex align-items-stretch">
        <nav id="sidebar">
            <div class="custom-menu">
                <button type="button" id="sidebarCollapse" class="btn btn-primary">
                    <i class="fa fa-bars"></i>
                    <span class="sr-only">Toggle Menu</span>
                </button>
            </div>
            <h1><a href="index.html" class="logo">Enlace Profesional</a></h1>
            <ul class="list-unstyled components mb-5">
                <li class="active">
                    <a href="adminNoticias.php"><span class="fa fa-home mr-3"></span> Noticias</a>
                </li>
                <li>
                    <a href="adminSuscriptores.php"><span class="fa fa-user mr-3"></span> Suscriptores</a>
                </li>
                <li>
                    <a href="adminComment.php"><span class="fa fa-sticky-note mr-3"></span> Comentarios</a>
                </li>
                <li>
                    <a href="administradores.php"><span class="fa fa-user mr-3"></span> Administradores</a>
                </li>
                <li>
                    <a href="adminclient.php"><span class="fa fa-user mr-3"></span> Clientes</a>
                </li>
                <li>
                    <a href="adminLogout.php"><span class="fa fa-sticky-note mr-3"></span> Cerrar Sesion</a>
                </li>
            </ul>
        </nav>
